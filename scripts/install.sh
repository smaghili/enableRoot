...
create_config_file() {
    ...
    if [[ "$OSTYPE" == "darwin"* ]]; then
        sed -i '' "s/YOUR_BOT_TOKEN_HERE/$BOT_TOKEN/g" ../config/config.json
        sed -i '' "s/YOUR_OPENROUTER_API_KEY_HERE/$OPENROUTER_KEY/g" ../config/config.json
        sed -i '' "s/123456789/$ADMIN_ID/g" ../config/config.json
    else
        sed -i "s/YOUR_BOT_TOKEN_HERE/$BOT_TOKEN/g" ../config/config.json
        sed -i "s/YOUR_OPENROUTER_API_KEY_HERE/$OPENROUTER_KEY/g" ../config/config.json
        sed -i "s/123456789/$ADMIN_ID/g" ../config/config.json
...
main() {
    ...
    if [ ! -f "../bot.py" ] || [ ! -f "../requirements.txt" ]; then
        print_error "Please run this script from the telegram-reminder-bot/scripts directory"
